...
class UniformMeshsize : public Meshsize {
 public:
  ...
  virtual FLOAT getPosY(int i, int j, int k) const {
    return _dy * (_firstCornerY - 2 + j);
  }
  virtual FLOAT getPosZ(int i, int j, int k) const {
    return _dz * (_firstCornerZ - 2 + k);
  }
 ...
 * stretching of the mesh can be introduced towards
 *  the outer boundaries, i.e. if stretchX is true (in constructor), then the
 * mesh will be finer close to the left and right
 *  boundary. The stretching is based on a formular involving tanh-functions, as
 * e.g. used in the dissertation by Tobias Neckel,
 *  chair of scientific computing (TUM).
 *  For non-stretched meshes, the UniformMeshsize implementation is used to
...
class TanhMeshStretching : public Meshsize {
 public:
  ...
  virtual FLOAT getDx(int i, int j) const {
    if (_stretchX) {
      ...
      return _uniformMeshsize.getDx(i, j);
    }
  }
  virtual FLOAT getDy(int i, int j) const {
    if (_stretchY) {
      ...
      return _uniformMeshsize.getDy(i, j);
    }
  }
  virtual FLOAT getDx(int i, int j, int k) const { return getDx(i, j); }
  virtual FLOAT getDy(int i, int j, int k) const { return getDy(i, j); }
  ...
  virtual FLOAT getPosX(int i, int j, int k) const {
    if (_stretchX) {
      ...
      return _uniformMeshsize.getPosX(i, j, k);
    }
  }
  virtual FLOAT getPosY(int i, int j, int k) const {
    if (_stretchY) {
      ...
      return _uniformMeshsize.getPosY(i, j, k);
    }
  }
  virtual FLOAT getPosZ(int i, int j, int k) const {
    if (_stretchZ) {
      ...
      return _uniformMeshsize.getPosZ(i, j, k);
    }
  }
  virtual FLOAT getPosX(int i, int j) const { return getPosX(i, j, 0); }
  virtual FLOAT getPosY(int i, int j) const { return getPosY(i, j, 0); }
 ...
 private:
  ...
  FLOAT computeCoordinate(int i, int firstCorner, int size, FLOAT length,
                          FLOAT dxMin) const {
    ...
    if (index < 0) {
      ...
      if (p < 0.5) {
        return 0.5 * length *
               (1.0 + tanh(_deltaS * (2.0 * p - 1.0)) / _tanhDeltaS);
  ...
  // 1D-cell
  FLOAT getMeshsize(int i, int firstCorner, int size, FLOAT length,
                    FLOAT dxMin) const {
    const FLOAT pos0 = computeCoordinate(i, firstCorner, size, length, dxMin);
    const FLOAT pos1 =
        computeCoordinate(i + 1, firstCorner, size, length, dxMin);
#ifdef DEBUG
    if (pos1 - pos0 < 1.0e-12) {
      handleError(1, "Error TanhMeshStretching::getMeshsize(): dx < 1.0e-12!");
    }
#endif
    return pos1 - pos0;
  }
