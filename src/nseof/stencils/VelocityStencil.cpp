...
void VelocityStencil::apply(FlowField& flowField, int i, int j) {
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) {  // If this is a fluid cell
    if ((obstacle & OBSTACLE_RIGHT) ==
        0) {  // Check whether the neighbor is also fluid
      ...
      velocity.getVector(i, j)[0] =
          ...
          dt / dx * (flowField.getPressure().getScalar(i + 1, j) -
                     flowField.getPressure().getScalar(i, j));

    } else {  // Otherwise, set to zero
      velocity.getVector(i, j)[0] = 0;
    }  // Note that we only set one direction per cell. The neighbor at the left
       // is
    ...
    if ((obstacle & OBSTACLE_TOP) == 0) {
      ...
      velocity.getVector(i, j)[1] =
          ...
          dt / dy * (flowField.getPressure().getScalar(i, j + 1) -
                     flowField.getPressure().getScalar(i, j));
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) {
    if ((obstacle & OBSTACLE_RIGHT) == 0) {
      ...
      velocity.getVector(i, j, k)[0] =
          ...
          dt / dx * (flowField.getPressure().getScalar(i + 1, j, k) -
    ...
    if ((obstacle & OBSTACLE_TOP) == 0) {
      ...
      velocity.getVector(i, j, k)[1] =
          ...
          dt / dy * (flowField.getPressure().getScalar(i, j + 1, k) -
                     flowField.getPressure().getScalar(i, j, k));
    } else {
