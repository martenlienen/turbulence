...
void MaxUStencil::cellMaxValue(FlowField& flowField, int i, int j, int k) {
  ...
  const FLOAT dx =
      FieldStencil<FlowField>::_parameters.meshsize->getDx(i, j, k);
  const FLOAT dy =
      FieldStencil<FlowField>::_parameters.meshsize->getDy(i, j, k);
  const FLOAT dz =
      FieldStencil<FlowField>::_parameters.meshsize->getDz(i, j, k);
  if (fabs(velocity[0]) / dx > _maxValues[0]) {
    _maxValues[0] = fabs(velocity[0]) / dx;
  }
  if (fabs(velocity[1]) / dy > _maxValues[1]) {
