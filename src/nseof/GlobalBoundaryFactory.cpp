...
GlobalBoundaryFactory::getGlobalBoundaryVelocityIterator(FlowField& flowField) {
  if (_parameters.geometry.dim == 2) {
    return GlobalBoundaryIterator<FlowField>(
        flowField, _parameters, *(_velocityStencils[0]),
        *(_velocityStencils[1]), *(_velocityStencils[2]),
        *(_velocityStencils[3]), 1, 0);
  }
  return GlobalBoundaryIterator<FlowField>(
      flowField, _parameters, *(_velocityStencils[0]), *(_velocityStencils[1]),
      *(_velocityStencils[2]), *(_velocityStencils[3]), *(_velocityStencils[4]),
      *(_velocityStencils[5]), 1, 0);
}

GlobalBoundaryIterator<FlowField>
GlobalBoundaryFactory::getGlobalBoundaryFGHIterator(FlowField& flowField) {
  if (_parameters.geometry.dim == 2) {
    return GlobalBoundaryIterator<FlowField>(
        flowField, _parameters, *(_FGHStencils[0]), *(_FGHStencils[1]),
        *(_FGHStencils[2]), *(_FGHStencils[3]), 1, 0);
  }
  return GlobalBoundaryIterator<FlowField>(
      flowField, _parameters, *(_FGHStencils[0]), *(_FGHStencils[1]),
