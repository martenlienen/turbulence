...
PetscParallelConfiguration::PetscParallelConfiguration(Parameters& parameters)
  ...
  if (nproc != nprocFromFile) {
    handleError(1,
                ...
                "doesn't match the communicator");
  }
}

PetscParallelConfiguration::~PetscParallelConfiguration() { freeSizes(); }
...
void PetscParallelConfiguration::createIndices() {
  ...
  _parameters.parallel.indices[0] =
      rank % _parameters.parallel.numProcessors[0];
  _parameters.parallel.indices[1] =
      (rank / _parameters.parallel.numProcessors[0]) %
      _parameters.parallel.numProcessors[1];
  _parameters.parallel.indices[2] =
      rank / (_parameters.parallel.numProcessors[0] *
              _parameters.parallel.numProcessors[1]);
}
