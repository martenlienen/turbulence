...
FLOAT computeVelocity3D(FlowField& flowField, int i, int j, int k,
  ...
  if (posY + 0.5 * dy >= stepSize) {
    ...
    return 36.0 * parameters.walls.vectorLeft[0] /
           (inletZSize * inletZSize * inletYSize * inletYSize) * y *
...
BFInputVelocityStencil::BFInputVelocityStencil(const Parameters& parameters)
    : BoundaryStencil<FlowField>(parameters),
      ...
      // configuration
      _stepSize(parameters.bfStep.yRatio > 0.0
                    ? parameters.bfStep.yRatio * parameters.geometry.lengthY
                    : 0.0) {
  if (_parameters.geometry.dim == 2) {
    ...
    for (int j = 0; j < _parameters.geometry.sizeY - 1; ++j) {
      ...
      if (posY + 0.5 * dy < _stepSize &&
          _stepSize <= posY + dy + 0.5 * nextDy) {
        _stepSize = posY + dy;
        break;
      }
    }
...
void BFInputVelocityStencil::applyRightWall(FlowField& flowField, int i,
                                            int j) {}
void BFInputVelocityStencil::applyBottomWall(FlowField& flowField, int i,
                                             int j) {}
...
void BFInputVelocityStencil::applyTopWall(FlowField& flowField, int i, int j,
                                          int k) {}
void BFInputVelocityStencil::applyFrontWall(FlowField& flowField, int i, int j,
                                            int k) {}
void BFInputVelocityStencil::applyBackWall(FlowField& flowField, int i, int j,
                                           int k) {}
...
void BFInputFGHStencil::applyTopWall(FlowField& flowField, int i, int j,
                                     int k) {}
void BFInputFGHStencil::applyFrontWall(FlowField& flowField, int i, int j,
                                       int k) {}
